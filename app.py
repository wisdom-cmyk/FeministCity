...
.panel { background:#fffdf9;border:1px solid var(--line);border-radius:13px;padding:1.3rem 1.45rem;min-height:235px; }.panel h3 { font-size:1.18rem;color:var(--orange);letter-spacing:.01em;margin:0 0 .7rem; }.panel p,.panel li { color:#354642;font-size:1rem;line-height:1.72; }.body-point { position:relative;padding-left:1.1rem; }.body-point:before { content:'•';position:absolute;left:0;color:var(--orange);font-weight:700; }
...
div.stButton>button { background:transparent;color:var(--ink);border:1px solid var(--ink);border-radius:999px;padding:.4rem 1.05rem;white-space:normal;height:auto; }div.stButton>button:hover { background:var(--ink);color:white;border-color:var(--ink); }.toc-title div.stButton>button { font-size:1.32rem;font-weight:700;text-align:left;border-radius:10px;background:#fffdf9;padding:1rem; }.toc-detail { padding:.7rem 1rem;border-left:2px solid var(--orange);line-height:1.65;color:#42534f; }.toc-list { padding-left:1.1rem;text-indent:-1.1rem;margin:.42rem 0; }.middle-title { font-size:1.55rem;font-weight:700;line-height:1.35;margin:0 0 1.1rem;color:#294d68; }.highlight { font-size:1.18rem;font-weight:600;line-height:1.68;color:#253c39;border-left:4px solid var(--orange);padding:.85rem 1.15rem;margin:0 0 1.35rem;background:#ece6d9; }.bilingual-ko { color:var(--muted);font-size:1.16rem;margin:-.35rem 0 1rem; }.author-box { background:#fffdf9;border:1px solid var(--line);border-radius:13px;padding:1rem; }.data-table { width:100%;border-collapse:collapse;margin-top:.7rem; }.data-table th,.data-table td { border:1px solid var(--line);padding:.75rem;vertical-align:top;text-align:left;line-height:1.55; }.data-table th { background:#e6ddd0;color:#294d68;width:28%; }
</style>
"""
...
def body_slide(slide, index):
    ...
    wide_pages = {5,7,8,9,11,17,19,21}
    items = slide["blocks"]
    if index == 12:
...
def intro(book, index):
    ...
    with right:
        ...
        with st.container(border=True):
            show_image(book["portrait"], use_container_width=True)
            st.markdown(f"<div class='lead'>{book['author']}</div>{book['bio']}", unsafe_allow_html=True)
...
def urban_matters():
    ...
    st.markdown(f"<div class='panel'>{DISCUSSION_TOPICS[0]['body']}</div>", unsafe_allow_html=True)
