...
def cover():
    st.markdown(f"<div style='min-height:66vh;display:flex;flex-direction:column;justify-content:center'><div class='cover-mark'>◒</div><div class='eyebrow'>{SESSION['course']} · {SESSION['week']}</div><div class='deck-title'>{SESSION['title']}</div><div class='subtitle'>{SESSION['subtitle']}</div><br><div class='meta'>{SESSION['date']} · {SESSION['presenters']}</div></div>", unsafe_allow_html=True)

def toc():
    st.markdown("<div class='eyebrow'>Contents</div><div class='slide-title'>목차</div><hr class='rule'>", unsafe_allow_html=True)
    if st.button("여자를 위한 도시는 없다", key="toc-kern", use_container_width=True): set_slide(2); st.rerun()
    st.markdown("&nbsp;&nbsp;1. 엄마들의 도시<br>&nbsp;&nbsp;2. 친구들의 도시<br>&nbsp;&nbsp;3. 혼자만의 도시<br>&nbsp;&nbsp;4. 시위의 도시<br><br>", unsafe_allow_html=True)
    if st.button("Discrimination by Design", key="toc-weisman", use_container_width=True): set_slide(BOOK2_INTRO); st.rerun()
    if st.button("Feminisms for our time! - Urban Matters Journal", key="toc-urban", use_container_width=True): set_slide(URBAN_INDEX); st.rerun()
...
def intro(book, index):
    ...
    with left: st.image(book["cover"], use_container_width=True)
    with middle: st.image(book["portrait"], use_container_width=True)
