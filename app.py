...
from content import BOOKS, DISCUSSION_SUBTOPICS, DISCUSSION_TOPICS, SESSION, SLIDES
...
def apply_emphasis(body):
    """발표 화면에서 지정 문장만 확실하게 강조한다."""
    rules = {
        "임신한 몸은 타인의 시선과 접촉의 대상이 되면서, 기존에 누렸던 도시 속 익명성·비가시성을 잃는다.": "<span style='text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px'>임신한 몸은 타인의 시선과 접촉의 대상이 되면서, 기존에 누렸던 도시 속 익명성·비가시성을 잃는다.</span>",
        "도시를 자유롭게 관찰하고 향유하는 ‘소요객’이라는 이상 자체가 돌봄의 책임에서 자유로운 몸을 기준으로 만들어진 개념일 수 있다.": "<span style='text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px'>도시를 자유롭게 관찰하고 향유하는 ‘소요객’이라는 이상 자체가 돌봄의 책임에서 자유로운 몸을 기준으로 만들어진 개념일 수 있다.</span>",
        "교외의 형성은 인종적 분리뿐 아니라 성별에 따른 공적/사적 영역과 유급/무급 노동의 분리를 강화했다.": "<span style='text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px'>교외의 형성은 인종적 분리뿐 아니라 성별에 따른 공적/사적 영역과 유급/무급 노동의 분리를 강화했다.</span>",
        "여성 친화적인 도시는 여성만을 위한 도시가 아니라 모든 사람을 환영하고 수용하는 곳이다.": "<span style='text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px'>여성 친화적인 도시는 여성만을 위한 도시가 아니라 모든 사람을 환영하고 수용하는 곳이다.</span>",
        "특정 거리나 시간대를 피하고 우회하게 되며, 그 결과 이동성과 사용할 수 있는 도시공간의 범위가 제한된다.": "<span style='text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px'>특정 거리나 시간대를 피하고 우회하게 되며, 그 결과 이동성과 사용할 수 있는 도시공간의 범위가 제한된다.</span>",
        ...
        "여성의 경험을 반영해 공공공간을 설계하는 것이 필요함": "<span style='text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px'>여성의 경험을 반영해 공공공간을 설계하는 것이 필요함</span>",
        "특히 거리에서 생활하는 여성은 일반적인 노숙의 위험에 더해 성폭력의 위험에도 더욱 크게 노출된다.": "<span style='text-decoration:underline;text-decoration-thickness:2px;text-underline-offset:4px'>특히 거리에서 생활하는 여성은 일반적인 노숙의 위험에 더해 성폭력의 위험에도 더욱 크게 노출된다.</span>",
    ...
    for plain, styled in rules.items():
...
def blocks(items, cols=2):
    ...
    for start in range(0, len(items), cols):
        ...
        for col, item in zip(row, items[start:start+cols]):
            with col: st.markdown(f"<div class='panel'><h3>{item['heading']}</h3>{apply_emphasis(item['body'])}</div>", unsafe_allow_html=True)
...
def discussion():
    ...
    st.markdown("<div class='panel'><div class='discussion-question'>• 페미니즘 관점으로 도시를 연구한다는 것은 무엇인가?</div><div class='discussion-question'>• 그 사회적·학술적 장점은 무엇인가?</div></div>", unsafe_allow_html=True)
    st.markdown("<br>", unsafe_allow_html=True)
    for subtopic in DISCUSSION_SUBTOPICS:
        st.markdown(f"<div class='subtopic'><h4>{subtopic['title']}</h4><p>{subtopic['body']}</p></div>", unsafe_allow_html=True)
    for topic in DISCUSSION_TOPICS[2:]:
        st.markdown(f"<div class='placeholder'>{topic['body']}</div><br>", unsafe_allow_html=True)
