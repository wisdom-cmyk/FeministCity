...
.panel { background:#fffdf9;border:2px solid var(--purple-dark);border-radius:20px;padding:1.3rem 1.45rem;min-height:165px;box-shadow:7px 7px 0 #e1bde7; }.panel h3 { font-size:1.22rem;color:var(--purple-dark);font-weight:800;letter-spacing:-.02em;margin:0 0 .7rem; }.panel p,.panel li { color:#332b37;font-size:1rem;line-height:1.72; }.body-point { position:relative;padding-left:1.1rem; }.body-point:before { content:'•';position:absolute;left:0;color:var(--purple);font-weight:700; }
...
def body_slide(slide, index):
    ...
    if index == 15:
        st.markdown("<table class='data-table'><tr><th>권력과 우월성</th><th>종속과 열등성</th></tr><tr><td>남성, 공적 공간·일터, 도시·문명, 위·앞·오른쪽, 이성·생산·통제</td><td>여성, 사적 공간·가정, 자연·황야, 아래·뒤·왼쪽, 감정·돌봄·의존</td></tr></table>", unsafe_allow_html=True)
    if index == 16:
...
def cover():
    ...
    with right:
        ...
        bread, rose = st.columns(2)
        with bread: show_image("assets/bread-hero.png", use_container_width=True)
        with rose: show_image("assets/rose-hero.png", use_container_width=True)
...
def discussion():
    ...
    for topic in DISCUSSION_TOPICS[1:]:
        if topic["status"] == "ready": st.markdown(f"<div class='panel' style='margin-bottom:1rem'><h3>{topic['heading']}</h3><div class='discussion-question'>{topic['body']}</div></div>", unsafe_allow_html=True)
...
def urban_matters():
    ...
    for image_name in ("assets/urban-matters-articles-1.png", "assets/urban-matters-articles-2.png", "assets/urban-matters-articles-3.png"):
        show_image(image_name, use_container_width=True)
